...
public class PartieV2 {
    ...
    private static int comptrévélés;
//constructeur 
...
//mélange des cartes
	...
	public static void protegerJoueur(int id) {
		...
		System.out.println("\nLe joueur "+id+" ne pourra pas etre accusé au prochain tour : "+PartieV2.partieUnique.jouerjoueur.get(id).peutEtreAccusé);
	}
    ...
    public static boolean revelerRole(int id) {
    	...
    	PartieV2.partieUnique.jouerjoueur.get(id).révélé=true;
...
//    	int nbjp=0;
    ...
    //choix d'avoir des bots ou non
    	...
    	if (choix == 'A') {
    		...
    		if(accusé>=0 && accusé<(PartieV2.partieUnique.getnbBots()+PartieV2.partieUnique.getnbJoueursPhys()) && accusé != actuel && PartieV2.partieUnique.jouerjoueur.get(accusé).révélé==false && PartieV2.partieUnique.jouerjoueur.get(accusé).peutEtreAccusé==true && PartieV2.partieUnique.jouerjoueur.get(accusé).enJeu==true) {
    		...
    		if(app==true && decision != 0){
    			...
    			PartieV2.partieUnique.jouerjoueur.get(idj).main.get(idchoisie).jouerCarte(nomcarte,true,actuel,actuel);
