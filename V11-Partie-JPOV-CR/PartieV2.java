...
//    	int nbjp=0;
    ...
    //choix d'avoir des bots ou non
    	...
    	if (choix == 'A') {
    		...
    		if(accusé>=0 && accusé<(PartieV2.partieUnique.getnbBots()+PartieV2.partieUnique.getnbJoueursPhys()) && accusé != actuel && PartieV2.partieUnique.jouerjoueur.get(accusé).révélé==false && PartieV2.partieUnique.jouerjoueur.get(accusé).peutEtreAccusé==true) {
        	...
        	if (réponse=='W') {
        		...
        		if (theinquisition.equalsIgnoreCase(nomc) && PartieV2.partieUnique.jouerjoueur.get(accusé).main.size()<2) {
    				app=false;
    		...
    		if (app==false) {
    			...
    			PartieV2.partieUnique.jouerjoueur.get(idj).main.get(idchoisie).jouerCarte(nomcarte,true,idj,actuel);
    		}
    		}
